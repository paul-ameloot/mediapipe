...
def _check_bazel():
  ...
  if not spawn.find_executable('bazelisk'):
...
class BuildBinaryGraphs(build.build):
  ...
  def _generate_binary_graph(self, graph_path):
    ...
    shutil.copyfile(
        os.path.join('bazelisk-bin/mediapipe/modules/', output_name), output_file)
...
class BuildBazelExtension(build_ext.build_ext):
  ...
  def bazel_build(self, ext):
    ...
    ext_bazel_bin_path = os.path.join('bazelisk-bin', ext.relpath,
                                      ext.target_name + '.so')
    ...
    if IS_WINDOWS:
      for opencv_dll in glob.glob(
          os.path.join('bazelisk-bin', ext.relpath, '*opencv*.dll')):
